function second() {
  ...
  for (var i = 3; i <= lastRow2; i++) {
    ...
    if (aa.getRange(i, 16).getValue() != "") {
      var itemNam = aa.getRange(i, 16).getValue();
      ...
      if (bb.getRange(o, 10).getValue() < 1) {
        SpreadsheetApp.getUi().alert("재고현황 단가 미입력");
        continue;
      }

      if (aa.getRange(i, 17).getValue() > sum) {
      ...
      var useSto = aa.getRange(i, 17).getValue();
      var useUnit = aa.getRange(i, 18);
      ...
      if (useSto <= oldSto[0]) {
        ...
        aa.getRange(i, 18).setValue(unitFin);
      }
      ...
      if (useSto > oldSto[0]) {
        ...
        var unitFin = (unitFin + unit2) / useSto; // unitFin에는 전체 amount를 다 더하고 그것을 사용한 재고량으로 나누었다.
